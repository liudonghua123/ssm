...
<mapper namespace="${table.domainClassName}">
	...
	<select id="select${table.domainClassName}List" resultMap="${table.domainClassName?uncap_first}ResultForList" parameterType="${table.domainClassName?uncap_first}">
		...
		<![CDATA[ ) t_ where rownum <= (#_rowBounds.offset# + #_rowBounds.limit#)) where rn_ >= (#_rowBounds.offset# + 1) ]]>
	</select>
	...
	<insert id="insert${table.domainClassName}" parameterType="${table.domainClassName?uncap_first}">
		<selectKey keyProperty="id" resultType="long" order="BEFORE">
		  select SEQ_TAB_${table.tableName}.nextval as id from dual
		</selectKey>
